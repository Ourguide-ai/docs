...
sequenceDiagram
    ...
    User->>OurguideCopilot: "Show me my mention rate chart"
    OurguideCopilot->>OurguideBackend: POST /chat/message
    OurguideBackend->>YourAPI: POST /api/tools/analytics-chart
    YourAPI-->>OurguideBackend: { chartType: "line", data: [...] }
    OurguideBackend-->>OurguideCopilot: SSE tool_result
    OurguideCopilot->>OurguideCopilot: toolRenderers["query_analytics_chart"]
    OurguideCopilot->>User: Renders custom chart inline
```
...
```tsx
import { OurguideCopilot } from '@ourguide/react';
import '@ourguide/react/styles.css';
...
```tsx
import type { ToolRenderers } from '@ourguide/react';
