---
title: "React Native"
description: "Add Argide to your React Native app"
---

The `@argide/react-native` package adds Argide to any React Native app. Drop in `<ArgideWidget />` for a ready-made floating chat button and bottom sheet, or use the `useChat` hook to build a fully custom interface.

## Installation

```bash
npm install @argide/react-native @argide/client
```
...
Add `<ArgideWidget />` inside your `<ArgideProvider>` for a floating chat button and bottom sheet — zero UI work required:

```jsx
import { ArgideProvider, ArgideWidget, createAsyncStorageAdapter } from '@argide/react-native';
...
export default function App() {
  return (
    <ArgideProvider productId="YOUR_PRODUCT_ID" storage={storage}>
      <YourApp />
      <ArgideWidget />
    </ArgideProvider>
  );
...
`ArgideWidget` renders a floating action button that opens a slide-up chat sheet. It handles messages, streaming, conversation history, and user auth out of the box.
...
```jsx
import { ArgideProvider, useChat } from '@argide/react-native';
...
## ArgideProvider Props

| Prop            | Required | Description                                                    |
| --------------- | -------- | -------------------------------------------------------------- |
| `productId`     | Yes      | Your product ID from the dashboard                             |
| `apiUrl`        | No       | API URL (defaults to `https://api.argide.ai`)         |
...
```jsx
import AsyncStorage from '@react-native-async-storage/async-storage';
import { createAsyncStorageAdapter } from '@argide/react-native';
...
```jsx
import { useConversations } from '@argide/react-native';
...
See [Identity Verification](/integration/identity-verification) for generating tokens on your backend.
...
```jsx
import { useArgideClient } from '@argide/react-native';

function App() {
  const client = useArgideClient();
...
Upload tool definitions on the [Actions](/agent/actions) page so the agent knows they exist.
...
export default function App() {
  ...
  return (
    <ArgideProvider productId={PRODUCT_ID} storage={storage}>
      <ChatScreen />
    </ArgideProvider>
  );
}
...
| Feature              | Web (`@argide/react-chat`)              | React Native (`@argide/react-native`)  |
| -------------------- | --------------------------------- | ---------------------------------------- |
...
| Provider             | `ArgideProvider` from `@argide/react-chat` | `ArgideProvider` from `@argide/react-native` |
