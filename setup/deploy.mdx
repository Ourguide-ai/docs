...
Go to [Deploy → Install](https://dashboard.argide.ai/dashboard/deploy/install) and copy your embed code:
...
Go to [Deploy → Backend Integration](https://dashboard.argide.ai/dashboard/deploy/backend-integration) and copy **ARGIDE_AUTH_SECRET**. Use it to sign JWTs for [Identity Verification](/integration/identity-verification).

<Warning>
  Keep your verification secret safe. Never expose it in client-side code.
</Warning>
...
- [ ] User attributes passed via `data-*` or `update`
- [ ] JWT endpoint implemented, if the agent calls your API on the user's behalf
