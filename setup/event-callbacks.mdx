...
Hook into the agent's lifecycle to build custom experiences on top of Argide.
...
```javascript
import { ArgideClient } from '@argide/client';

const client = new ArgideClient({
...
client.on({
  onToolCall: (event) => {
    if (event.type === 'tool_call_complete') {
      analytics.track('argide_tool_used', {
        tool: event.toolName,
