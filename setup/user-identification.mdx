...
To keep the widget hidden until the user signs in, add `data-visibility="false"` to the script tag and reveal it in the same `update` call:
...
See [Visibility](/agent/visibility) for the full behaviour of `data-visibility` and the `visibility` property.
...
An `update` **merges**. A property you pass replaces its current value, and a property you leave out keeps its current value. A partial update can never wipe attributes you set earlier:
...
`visibility` is the exception: it only affects the widget in the browser and is never sent to Argide.
