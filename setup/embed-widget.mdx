...
<AccordionGroup>
  ...
  <Accordion title="How do I tell the agent who the user is?">
    Pass the user's attributes:
    ...
    Their name then appears on the conversation in Activity. On a multi-page site you can put the same values on the script tag as `data-user-id`, `data-name`, and `data-email`.

    See [User Identification](/setup/user-identification).
  </Accordion>

  <Accordion title="How do I let the agent call my API as the user?">
    Two steps:
    ...
    Argide forwards that token to your API on every tool call, so the agent can act on the user's behalf.

    See [Identity Verification](/integration/identity-verification) for the backend JWT setup.
  </Accordion>

  <Accordion title="How do I hide the widget until the user signs in?">
    Add `data-visibility="false"` to the script tag, then reveal it after login:

    ```javascript
    window.argide('update', { visibility: true });
    ```

    Until then the widget doesn't appear and doesn't connect. See [Visibility](/agent/visibility).
  </Accordion>
