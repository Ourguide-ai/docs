...
<Tabs>
  ...
  <Tab title="React SDK">
    ...
    | `apiUrl`           | Yes      | `https://api.ourguide.ai`                                                                                         |
    ...
    <Accordion title="Copilot Sidebar">
      A full-height sidebar that automatically pushes your page content aside. Wrap your app with `OurguideProvider` and place `OurguideCopilot` alongside your app:

      ```bash
      npm install @ourguide/client @ourguide/ui
      ```

      ```tsx
      import { OurguideProvider, OurguideCopilot } from '@ourguide/ui';
      import '@ourguide-ai/ui/styles.css';
      ...
      function App() {
        return (
          <OurguideProvider
            productId="YOUR_PRODUCT_ID"
            apiUrl="https://dashboard.ourguide.in"
          >
            <YourApp />
...
1. Go to [dashboard.ourguide.ai/dashboard/deploy/widget](https://dashboard.ourguide.ai/dashboard/deploy/widget)
...
<Tip>
  `window.ourguide('open')` works from anywhere — event handlers, `useEffect`, timeouts, or third-party callbacks. No imports needed.
</Tip>
