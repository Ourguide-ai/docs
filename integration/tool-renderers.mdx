...
sequenceDiagram
    ...
    User->>ArgideCopilot: "Show me my mention rate chart"
    ArgideCopilot->>ArgideBackend: POST /chat/message
    ArgideBackend->>YourAPI: POST /api/tools/analytics-chart
    YourAPI-->>ArgideBackend: { chartType: "line", data: [...] }
    ArgideBackend-->>ArgideCopilot: SSE tool_result
    ArgideCopilot->>ArgideCopilot: toolRenderers["query_analytics_chart"]
    ArgideCopilot->>User: Renders custom chart inline
```
...
```tsx
import { ArgideCopilot } from '@argide/react-chat';
...
```tsx
import type { ToolRenderers } from '@argide/react-chat';
