...
3. Argide routes API calls to the matching base URL with the matching credentials

<Info>
  If no subdomain is passed, Argide falls back to the default base URL and auth config — fully backward compatible.
</Info>
...
```bash
curl -X PATCH https://api.argide.ai/api/products/YOUR_PRODUCT_ID/endpoint-configs \
...
```bash
curl -X PUT https://api.argide.ai/api/products/YOUR_PRODUCT_ID/endpoint-configs \
  -H "Content-Type: application/json" \
  -d '{
    "configs": {
      "workspace1": {
...
```bash
curl -X PUT https://api.argide.ai/api/products/YOUR_PRODUCT_ID/endpoint-configs/workspace1 \
  -H "Content-Type: application/json" \
  -d '{
    "api_base_url": "https://workspace1.app.com/api",
    "api_auth_config": { "auth_type": "bearer", "token": "token-for-ws1" }
...
```bash
curl -X DELETE https://api.argide.ai/api/products/YOUR_PRODUCT_ID/endpoint-configs/workspace1
```

### List All

```bash
curl https://api.argide.ai/api/products/YOUR_PRODUCT_ID/endpoint-configs
```
...
Tell the widget which subdomain to use so Argide routes API calls to the correct endpoint.

<CodeGroup>

```html Script Tag
<script
  src="https://dashboard.argide.ai/static/argide-widget.js"
