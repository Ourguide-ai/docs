...
<Note>
  This is the standard [Identity Verification](/setup/identity-verification) flow. If you haven't set it up, users are treated as anonymous and identity headers won't be available.
</Note>
...
<Note>
  Identity claims are only available for authenticated users. If the user hasn't been identified via [Identity Verification](/setup/identity-verification), identity headers won't be sent.
</Note>
...
<Check>Set up [Identity Verification](/setup/identity-verification) if your tools need to know who the user is.</Check>
...
| Identity headers are empty        | User not authenticated in the widget| Set up [Identity Verification](/setup/identity-verification), ensure frontend calls `identify()` |
