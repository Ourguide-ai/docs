...
<Info>
  This is not your app's session JWT. It's an Argide-scoped identity token your backend mints with `ARGIDE_AUTH_SECRET`. See [Identity Verification](/integration/identity-verification).
</Info>
...
function authMiddleware(req, res, next) {
  ...
  try {
    const payload = jwt.verify(token, process.env.ARGIDE_AUTH_SECRET);
...
def auth_required(f):
    ...
    async def decorated(request, *args, **kwargs):
        ...
        try:
            payload = jwt.decode(token, os.environ["ARGIDE_AUTH_SECRET"], algorithms=["HS256"])
