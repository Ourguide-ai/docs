...
<Note>
  Tokens expire. Re-mint before `exp` passes, or Argide has no valid token to forward and your API returns 401. With a one-hour token, refresh every 50 minutes:
